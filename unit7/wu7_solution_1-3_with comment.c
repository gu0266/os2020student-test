...
// logical sectors entries at offset 0x020 ~ 0x200
const unsigned short sector_entries_start = 0x020;
const unsigned short sector_entries_end = 0x200;
const short ZERO = 0x0;

u_int32_t extract_uint32(int offset)
{
  return sector_buffer[offset + 0] + (sector_buffer[offset + 1] << 8) + (sector_buffer[offset + 2] << 16) + (sector_buffer[offset + 3] << 24);
}

u_int32_t extract_uint16(int offset)
{
  return sector_buffer[offset + 0] + (sector_buffer[offset + 1] << 8);
}
...
{
  // First, read the Master Boot Record, which is in the first sector of the disk.
  ...
  sdcard_readsector(0);
  // Partition entry will begin at one of $1be, $1ce, $1de or $1ee within that sector.
  // Each size is 16(0x010) bytes
  for (int i = 0x1be; i < 0x1fe; i += 0x010)
  {
  	// A FAT32 partition have a partition type field of $0x0C.
  	// Number of sectors in partition is 4 bytes.
    if (extract_uint32(i + 4) == 0x0c)
    {
      // The location of the partition N entry in the MBR +
      // The offset in a Partition Entry
      // Store above value into the pre-defined variables p_start and p_size.
      p_start = extract_uint32(i + 0x08);	// First absolute sector in the partition.
      p_size = extract_uint32(i + 0x0c); // Number of sectors in partition.
    }
  }
  ...
  // Read the first sector of the FAT32 partition by using extract_uint32(), extract_uint16(), or sector_buffer[].
  sdcard_readsector(p_start);
  // The number of 512 byte sectors in each FAT.
  ...
  // The number of reserved sectors on the file system from the BIOS Parameter Block. Number of reserved sectors.
  f_reserved_sectors = extract_uint16(0x0e);
  // The number of sectors in each cluster from the BIOS Parameter Block.
  ...
  // The absolute sector number of the first FAT. The sector number relative to the start of the partition, plus the starting point of the partition.
  f_fat1_sector = p_start + f_reserved_sectors;
  // The absolute sector number of the second FAT. The sector number relative to the start of the partition, plus the starting point of the partition
  f_fat2_sector = f_fat1_sector + f_sectors_per_fat;
  // The absolute sector number of the root directory of the file system. The sector number relative to the start of the partition, plus the starting point of the partition
  f_rootdir_sector = f_fat2_sector + f_sectors_per_fat;
  // The number of clusters in the file system.
  // f_clusters can be calculated simple as the number of sectors per fat multiplied by the number of 32-bit values (i.e., 4 bytes long each)
...
{
  // Use the three convenient variables above to point to the start of
...
{
  // First, find the next directory entry in the directory that is valid.
  ...
  while (dir_sector <= dir_sector_max)
  {
    ...
    {
      // A valid directory entry has a file name in it that doesn't begin with the null character or the special chartacter 0xe5.
      ...
      // logical sectors entries at offset 0x020.
      dir_sector_offset += sector_entries_start;
    }

    // If directory entry has a file name, read the next sector.
  ...
  // Directory entry - File Attributes
  j = dir_sector_offset + 0x0b;
  return_structure.attribs = sector_buffer[j];
  // The lower two bytes of the file content start cluster number is 0x1a~0x1b and the higher two bytes at 0x14~0x15 make up the file content start cluster number.
  // Get the cluster from the halves of the cluster number are located and bit-shifting to combine the two halves to make a valid cluster number.
  ...
  // Finally, advance dir_sector_offset (and dir_sector if the offset goes past the end
  // of the sector), so that it is pointing at the next directory entry, ready for the next call
  // to this function.
  dir_sector_offset += sector_entries_start;
  if (dir_sector_offset >= sector_entries_end)
  {
    dir_sector++;
    dir_sector_offset = ZERO;
  }
  ...
  // And really last of all, return a pointer to the return_structure.
